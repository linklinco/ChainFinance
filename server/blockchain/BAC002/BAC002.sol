...
contract BAC002 is IssuerRole, Suspendable {
    ...
    function balance(address owner) public view returns (uint256) {
        require(
            owner != address(0),
            "BAC002: balance query for the zero address"
        );
    ...
    function ownerOf(uint256 assetId) public view returns (address) {
        ...
        require(
            owner != address(0),
            "BAC002: owner query for nonexistent asset"
        );
        return owner;
    }

    function assetOfOwnerByIndex(
        address owner,
        uint256 index
    ) public view returns (uint256) {
        require(
            index < balance(owner),
            "BAC002Enumerable: owner index out of bounds"
        );
    ...
    function assetByIndex(uint256 index) public view returns (uint256) {
        require(
            index < totalSupply(),
            "BAC002Enumerable: global index out of bounds"
        );
    ...
    function getApproved(uint256 assetId) public view returns (address) {
        require(
            _exists(assetId),
            "BAC002: approved query for nonexistent asset"
        );
    ...
    function sendFrom(
        ...
        require(
            _isApprovedOrOwner(msg.sender, assetId),
            "BAC002: send caller is not owner nor approved"
        );
        _sendFrom(from, to, assetId, data);
        require(
            _checkOnBAC002Received(from, to, assetId, data),
            "BAC002: send to non BAC002Receiver implementer"
        );
    }

    function batchSendFrom(
        address from,
        address[] to,
        uint256[] assetId,
        bytes memory data
        ...
        require(
            to.length == assetId.length,
            "to and assetId array lenght must match."
        );
    ...
    function destroy(uint256 assetId, bytes data) public {
        ...
        require(
            _isApprovedOrOwner(msg.sender, assetId),
            "BAC002Burnable: caller is not owner nor approved"
        );
        _destroy(assetId, data);
    }

    //add issuerAddress
    function issueWithAssetURI(
        address to,
        uint256 assetId,
    ...
    function assetURI(uint256 assetId) external view returns (string memory) {
        require(
            _exists(assetId),
            "BAC002Metadata: URI query for nonexistent asset"
        );
    ...
    function _setAssetURI(uint256 assetId, string memory uri) internal {
        require(
            _exists(assetId),
            "BAC002Metadata: URI set of nonexistent asset"
        );
    ...
    function _isApprovedOrOwner(
        ...
        require(
            _exists(assetId),
            "BAC002: operator query for nonexistent asset"
        );
        address owner = ownerOf(assetId);
    ...
    function _destroy(address owner, uint256 assetId, bytes data) internal {
        require(
            ...
            "BAC002: destroy of asset that is not own"
        );
     ...
     */
    function _removeAssetFromOwnerEnumeration(
        address from,
        uint256 assetId
    ) private {
    ...
    function _sendFrom(
        ...
        require(
            ownerOf(assetId) == from,
            "BAC002: send of asset that is not own"
        );
     ...
     */
    function _checkOnBAC002Received(
        address from,
        address to,
        uint256 assetId,
        bytes memory _data
        ...
        bytes4 retval = IBAC002Receiver(to).onBAC002Received(
            msg.sender,
            from,
            assetId,
            _data
